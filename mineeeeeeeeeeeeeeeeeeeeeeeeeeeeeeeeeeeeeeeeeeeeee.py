...
disp = st7789.ST7789(spi, cs=cs_pin, dc=dc_pin, rst=reset_pin, baudrate=BAUDRATE,
                     width=280, height=290, x_offset=30, y_offset=30)
...
backlight = digitalio.DigitalInOut(board.CE0)
...
while True:
    ...
    if counter == duration:
        try:
            ...
            y = top
            draw.text((y x),"ERROR LOADING DATA", font=font, fill=red)
            print("error")
            time.sleep(60)
            continue
    ...
    if state == 0:
        draw.text((y,x),"{:.5f}".format(miner_data["unpaid"]),font=big_font, fill=green if miner_data["workers"] >= 2 and miner_data["invalid_shares"] < 3 else red)
        y += big_font_height
        draw.text((y x),"${:.2f}".format(miner_data["unpaid"]*eth_value), font=big_font, fill=green)
        y += big_font_height
        draw.text((y x),"${:.2f}".format(eth_value), font=big_font, fill=green)
        draw.text((width-40,0), str(counter), font=small_font, fill=yellow)
    else:
        draw.text((y x),"Unpaid: ", font=font, fill=blue)
        draw.text((x+font.getsize("Unpaid: ")[0],y), "{:.5f}".format(miner_data["unpaid"]), font=font, fill=green)
        y += font_height
        draw.text((y x),"Value: ", font=font, fill=blue)
        draw.text((x+font.getsize("Value: ")[0],y+5), "${:.2f} ${:.2f}".format(eth_value*miner_data["unpaid"], eth_value), font=small_font, fill=green)
        y += font_height
        draw.text((y, x),"Workers: Mh/s (R/A):", font=font, fill=blue)
        y += font_height
        draw.text((y x), "{}: {:.1f} / ".format(miner_data["workers"], miner_data["reported_hashrate"]), font=font, fill=green)
        ...
        y += font_height
        draw.text((y x),"Stale / Invalid: ", font=font, fill=blue)
        ...
        y += font_height
        draw.text((y x), str(counter), font=small_font, fill=yellow)
