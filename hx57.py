...
if counter == duration:
        try:
            ...
            eth_value = data["ethereum"]["usd"]
            y = top
            draw.text((x+font.getsize("Refreshing in: ")[0],0), str(counter), font=small_font, fill=purple)
            draw.text((x+font.getsize("Unpaid Amount:  ")[0],25), "{:.5f}".format(miner_data["unpaid"]), font=font, fill=green)
            y += font_height
            draw.text((x,50),"Earned/ETH Value: ", font=big_font, fill=purple)
            ...
            draw.text((x+font.getsize("{} - { Worker(s):.1f} ".format(miner_data["workers"], miner_data["reported_hashrate"]))[0],100), "{:.1f}".format(miner_data["actual_hashrate"]), font=big_font, fill=green if miner_data["actual_hashrate"] >= miner_data["reported_hashrate"] else yellow)
            y += font_height
            draw.text((x,350),"Stale / Invalid: ", font=big_font, fill=blue)
            draw.text((x+font.getsize("Stale / Invalid: ")[0],350), str(miner_data["stale_shares"]), font=big_font, fill=green)
            draw.text((x+font.getsize("Stale / Invalid: {}".format(miner_data["stale_shares"]))[0],350), " / ", font=big_font, fill=blue)
            draw.text((x+font.getsize("Stale / Invalid: {} / ".format(miner_data["stale_shares"]))[0],350),str(miner_data["invalid_shares"]), font=big_font, fill=green if miner_data["invalid_shares"] == 0 else red)
            y += font_height
        except:
            y = top
            draw.text((x,y),"ERROR LOADING DATA", font=font, fill=red)
