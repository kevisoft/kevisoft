...
disp = st7789.ST7789(spi, cs=cs_pin, dc=dc_pin, rst=reset_pin, baudrate=BAUDRATE,
                     width=320, height=480, x_offset=0, y_offset=0)
...
backlight = digitalio.DigitalInOut(board.GPIO-4)
backlight.switch_to_output()
backlight.value = True

# Add buttons as inputs
buttonA = digitalio.DigitalInOut(board.A0)
...
while True:
    ...
    if state == 0:
        ...
        draw.text((x+font.getsize("Refreshing in: ")[0],y+7), str(counter), font=small_font, fill=yellow)
		
    else:
        draw.text((x,y),"Unpaid: ", font=font, fill=purple)
