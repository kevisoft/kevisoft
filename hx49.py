...
if counter == duration:
        try:
            ...
            eth_value = data["ethereum"]["usd"]
            y = top
            draw.text((x+font.getsize("Refreshing in: ")[0],0), str(counter), align="left", font=small_font, fill=purple)
			draw.text((x,y), str(counter), font=small_font, fill=yellow)
            draw.text((x,25),"Unpaid Amount: ", font=font, align="right", fill=purple)
            draw.text((x+font.getsize("Unpaid Amount:  ")[0],25), "{:.5f}".format(miner_data["unpaid"]), font=font, fill=green)
            y += font_height
            draw.text((x,50),"Total Earned: ", font=font, fill=purple)
            draw.text((x+font.getsize("Value: ")[0],50), "${:.2f} ${:.2f}".format(eth_value*miner_data["unpaid"], eth_value), font=small_font, fill=green)
            y += font_height
            draw.text((x,75),"Hashrate (Mh/s):", font=big_font, fill=blue)
            y += font_height
            draw.text((x,250), "{}: {:.1f} / ".format(miner_data["workers"], miner_data["reported_hashrate"]), font=small_font, fill=green)
            draw.text((x+font.getsize("{} - {:.1f} / ".format(miner_data["workers"], miner_data["reported_hashrate"]))[0],250), "{:.1f}".format(miner_data["actual_hashrate"]), font=big_font, fill=green if miner_data["actual_hashrate"] >= miner_data["reported_hashrate"] else yellow)
            y += font_height
            draw.text((x,350),"Stale / Invalid: ", font=font, fill=blue)
            draw.text((x+font.getsize("Stale / Invalid: ")[0],350), str(miner_data["stale_shares"]), font=font, fill=green)
            draw.text((x+font.getsize("Stale / Invalid: {}".format(miner_data["stale_shares"]))[0],350), " / ", font=font, fill=blue)
