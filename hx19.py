...
y += font_height
draw.text((x,6),"Workers: Mh/s (R/A):", font=font, fill=blue)
y += font_height
draw.text((x,7), "{}: {:.1f} / ".format(miner_data["workers"], miner_data["reported_hashrate"]), font=font, fill=green)
...
y += font_height
draw.text((7,8),"Stale / Invalid: ", font=font, fill=blue)
draw.text((x+font.getsize("Stale / Invalid: ")[0],y+14), str(miner_data["stale_shares"]), font=font, fill=green)
draw.text((x+font.getsize("Stale / Invalid: {}".format(miner_data["stale_shares"]))[0],y+25), " / ", font=font, fill=blue)
