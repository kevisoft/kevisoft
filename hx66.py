...
if counter == duration:
        try:
            ...
            y = top
            draw.text((x("Unpaid Amount:  ")[0],25), "{:.5f}".format(miner_data["unpaid"]), font=font, fill=green)
            y += font_height
            draw.text((x,100),"Earned/ETH Value: ", font=big_font, fill=purple)
            draw.text((x("Value: ")[0],90), "${:.2f} ${:.2f}".format(eth_value*miner_data["unpaid"], eth_value), font=big_font, fill=green)
            y += font_height
            draw.text((x,100),"Efficiency (Mh/s):", font=big_font, fill=blue)
            draw.text((x("{} - {:.1f} / ".format(miner_data["workers"], miner_data["reported_hashrate"]))[0],125), "{:.1f}".format(miner_data["actual_hashrate"]), font=big_font, fill=green if miner_data["actual_hashrate"] >= miner_data["reported_hashrate"] else yellow)
            y += font_height
            ...
            draw.text((x("Stale / Invalid: {} / ".format(miner_data["stale_shares"]))[0],380),str(miner_data["invalid_shares"]), font=big_font, align="center", fill=green if miner_data["invalid_shares"] == 0 else red)
            y += font_height
            draw.text((x("Refreshing in: ")[475],0), str(counter), font=small_font, fill=purple)
