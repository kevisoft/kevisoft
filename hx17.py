...
y += big_font_height
draw.text((x,2),"${:.2f}".format(miner_data["unpaid"]*eth_value), font=big_font, fill=green)
y += big_font_height
draw.text((x,3),"${:.2f}".format(eth_value), font=big_font, fill=green)
draw.text((x+font.getsize("Refreshing in: ")[0],y+1), str(counter), align="left", font=small_font, fill=yellow)
draw.text((x,4),"Unpaid: ", font=font, align="right", fill=purple)
draw.text((x+font.getsize("Unpaid: ")[0],y), "{:.5f}".format(miner_data["unpaid"]), font=font, fill=green)
y += font_height
draw.text((x,5),"Value: ", font=font, fill=purple)
