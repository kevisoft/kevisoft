...
def get_flexpool_values(wallet):
    ...
    ret_value["actual_hashrate"] = data["result"]["current"]["effective_hashrate"]/1000000            y = top
...
big_font = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf', 20)
...
if counter == duration:
        try:
            ...
            y = top
            draw.text((x,1),"{:.5f}".format(miner_data["unpaid"]),font=big_font, fill=green if miner_data["workers"] >= 2 and miner_data["invalid_shares"] < 3 else red)
            y += big_font_height
            draw.text((x,50),"${:.2f}".format(miner_data["unpaid"]*eth_value), font=big_font, fill=green)
            y += big_font_height
            draw.text((x,100),"${:.2f}".format(eth_value), font=big_font, fill=green)
            draw.text((x+font.getsize("Refreshing in: ")[0],y+1), str(counter), align="left", font=small_font, fill=purple)
            draw.text((x,150),"Unpaid: ", font=font, align="right", fill=purple)
            draw.text((x+font.getsize("Unpaid: ")[0],150), "{:.5f}".format(miner_data["unpaid"]), font=font, fill=green)
            y += font_height
            draw.text((x,200),"Value: ", font=font, fill=purple)
            draw.text((x+font.getsize("Value: ")[0],203), "${:.2f} ${:.2f}".format(eth_value*miner_data["unpaid"], eth_value), font=small_font, fill=green)
            y += font_height
            draw.text((x,250),"Workers: Mh/s (R/A):", font=big_font, fill=blue)
            y += font_height
            draw.text((200,250), "{}: {:.1f} / ".format(miner_data["workers"], miner_data["reported_hashrate"]), font=small_font, fill=green)
            draw.text((250+font.getsize("{} - {:.1f} / ".format(miner_data["workers"], miner_data["reported_hashrate"]))[0],250), "{:.1f}".format(miner_data["actual_hashrate"]), font=big_font, fill=green if miner_data["actual_hashrate"] >= miner_data["reported_hashrate"] else yellow)
            y += font_height
            draw.text((x,350),"Stale / Invalid: ", font=font, fill=blue)
            draw.text((x+font.getsize("Stale / Invalid: ")[0],350), str(miner_data["stale_shares"]), font=font, fill=green)
            draw.text((x+font.getsize("Stale / Invalid: {}".format(miner_data["stale_shares"]))[0],350), " / ", font=font, fill=blue)
            draw.text((x+font.getsize("Stale / Invalid: {} / ".format(miner_data["stale_shares"]))[0],350),str(miner_data["invalid_shares"]), font=font, fill=green if miner_data["invalid_shares"] == 0 else red)
            y += font_height
            draw.text((x,350), str(counter), font=small_font, fill=yellow)
        except:
            y = top
            draw.text((x,y),"ERROR LOADING DATA", font=font, fill=red)
            print("error")
            time.sleep(60)# -*- coding: utf-8 -*-
import sys
import time
import json
import requests
...
if counter == duration:
        try:
            ...
            eth_value = data["ethereum"]["usd"]
            y = top
            draw.text((x+font.big_font("Unpaid Amount:  ")[0],25), "{:.5f}".format(miner_data["unpaid"]), font=font, fill=green)
            y += font_height
            draw.text((x,100),"Earned/ETH Value: ", font=big_font, fill=purple)
            draw.text((x+font.big_font("Value: ")[0],90), "${:.2f} ${:.2f}".format(eth_value*miner_data["unpaid"], eth_value), font=big_font, fill=green)
            y += font_height
			draw.text((x,100),"Efficiency (Mh/s):", font=big_font, fill=blue)
            draw.text((x+font.big_font("{} - {:.1f} / ".format(miner_data["workers"], miner_data["reported_hashrate"]))[0],125), "{:.1f}".format(miner_data["actual_hashrate"]), font=big_font, fill=green if miner_data["actual_hashrate"] >= miner_data["reported_hashrate"] else yellow)
            y += font_height
            ...
            draw.text((x+font.big_font("Stale / Invalid: {} / ".format(miner_data["stale_shares"]))[0],380),str(miner_data["invalid_shares"]), font=big_font, align="center", fill=green if miner_data["invalid_shares"] == 0 else red)
            y += font_height
			draw.text((x+font.big_font("Refreshing in: ")[475],0), str(counter), font=small_font, fill=purple)
